...
DATASET_NAME_TO_VERSION = {
    ...
    # Custom dataset support: use all scenes available under the given dataroot
    # without relying on official nuScenes split names.
    "nusc_custom": "v1.0-trainval",
}
...
class NuscenesDataset(Dataset):
    ...
    def _get_split_scenes(self, name):
        # Support a custom mode where we simply enumerate all scenes present
        # in the provided dataroot without relying on official scene names.
        if name == "nusc_custom":
            # Use all scenes that exist in the current dataroot/version
            return [(scene['name'], scene['token']) for scene in self.nusc.scene]
        ...
        # Filter out any scene names that do not exist in current dataroot
        scenes = [n for n in scenes if n in name_to_token]
