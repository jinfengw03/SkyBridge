...
def load_gt_from_data_infos(nusc: NuScenes, data_infos, box_cls, verbose: bool = False):
    """
    Loads ground truth boxes based on the provided data_infos list instead of official nuScenes splits.
    This is useful for custom datasets/splits where scene names do not match the official split lists.
    :param nusc: A NuScenes instance.
    :param data_infos: Iterable of data infos; each item must contain a 'token' of the sample.
    :param box_cls: Type of box to load, e.g. DetectionBox or TrackingBox.
    :param verbose: Whether to print messages to stdout.
    :return: The GT boxes.
    """
    ...
    # Build an index map for per-sample ordering like in the default loader.
    index_map = {}
    for scene in nusc.scene:
        first_sample_token = scene['first_sample_token']
        sample = nusc.get('sample', first_sample_token)
    ...
    # Collect sample tokens from data_infos, keeping only those present in the DB.
    sample_tokens = []
    tokens_in_db = {s['token'] for s in nusc.sample}
    for info in data_infos:
        st = info.get('token', None)
        if st is None:
            continue
        if st in tokens_in_db:
            sample_tokens.append(st)

    all_annotations = EvalBoxes()

    # Load annotations per sample.
    for sample_token in tqdm.tqdm(sample_tokens, leave=verbose):
        sample = nusc.get('sample', sample_token)
        sample_annotation_tokens = sample['anns']
            ...
            if box_cls == DetectionBox_modified:
                ...
                sample_boxes.append(
                    box_cls(
                        ...
                        attribute_name=attribute_name,
                        visibility=sample_annotation['visibility_token'],
                        index=index_map.get(sample_token, 0)
                    )
                )
            elif box_cls == TrackingBox:
                assert False
    ...
    if verbose:
        print("Loaded ground truth annotations (custom split) for {} samples.".format(len(all_annotations.sample_tokens)))
...
class NuScenesEval_custom(NuScenesEval):
    ...
    def __init__(self,
        ...
        self.pred_boxes, self.meta = load_prediction(self.result_path, self.cfg.max_boxes_per_sample, DetectionBox,
                                                     verbose=verbose)
        # Prefer building GT from provided data_infos (custom split) if available; otherwise fall back to official split.
        if self.data_infos is not None and len(self.data_infos) > 0:
            self.gt_boxes = load_gt_from_data_infos(self.nusc, self.data_infos, DetectionBox_modified, verbose=verbose)
        else:
        ...
        if pred_tokens != gt_tokens:
            inter_tokens = pred_tokens & gt_tokens
            if self.verbose:
                only_pred = len(pred_tokens - gt_tokens)
                only_gt = len(gt_tokens - pred_tokens)
                print(f"Warning: prediction/GT sample mismatch. Using intersection. "
                      f"pred_only={only_pred}, gt_only={only_gt}, inter={len(inter_tokens)}")
            # If no overlap at all, keep the original assert for a clear failure mode.
            assert len(inter_tokens) > 0, \
                "No overlapping samples between predictions and GT; check split/version/result file."
            # Reduce to intersection so training-time eval can proceed on partial predictions.
            self.pred_boxes = filter_by_sample_token(self.pred_boxes, inter_tokens, verbose=False)
            self.gt_boxes = filter_by_sample_token(self.gt_boxes, inter_tokens, verbose=False)
